	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			try {
				...
				const commands = this.getNodeParameter('golemCommands', itemIndex, '') as {commandsValues: {command: string; commandText: string}[]}
							...
							for (const commandObj of commands.commandsValues) {
      						...
      						await ctx.run("cp /golem/work/File.csv /golem/output/File.csv")
									...
									item.binary = {
										...(item.binary || {}),
										//@ts-ignore
										[outputFieldName]: {mimeType: commandObj.mimeType, data: file.toString('base64'), fileType: commandObj.fileType, fileName: 'superFile.csv'}
									}
								}
							}
							console.log('DONE')
